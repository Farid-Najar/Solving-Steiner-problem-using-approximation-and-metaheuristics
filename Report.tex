...
\begin{document}
	...
	\section{Résultats et performances}
        ...
        Le graphique suivant ({\color{red} best b2 evaluation diff recuit 2000 20000}) montre en particulier la différence de convergence entre un algorithme de recuit avec température initiale 2000 ou 20000. Ce graphique nous montre en particulier que la température initiale va modifier la vitesse de convergence de l'algorithme. Plus la température est élevée, plus le début sera lent. Par ailleurs, plus la température initiale est élevée, plus le temps d'éxecution de l'algorithme sera long pour un résultat relativement identique.
        Par conséquent, il est préférable, dans une circonstance de rapidité d'éxecution de l'algorithme, d'initialiser la température à 2000.
        En revanche, bien que la convergence ait toujours lieu avec des températures initiales inférieures, fixer la température à 2000 est un juste milieu entre le temps d'execution de l'algorithme et la stabilité des solution. En effet, une température inférieure donnera plus de chance à une divergence de la solution.

        \subsubsection{Paramètre lambda}
        La température initiale fixée à 2000, on compare dans les graphes suivants la convergence du recuit pour \lambda fixé à 0.99 ou 0.999 sans variation interne ({\color{red} best b2 evaluation diff recuit 0.99 0.999}) et la convergence du recuit pour \lambda fixé à 0.99 l'un sans variation interne et l'autre avec ({\color{red} best b2 evaluation diff recuit 0.99 change}).
        ...
        Le graphe suivant compare l'algorithme de recuit à celui de recuit multiple pour un nombre de chercheur fixé à 5
        ({\color{red} best b2 evaluation diff multiple simple})
        ...
        Pour cet algorithme, il est possible de faire varier ses performances en faisant varier l'initialisation (fonction $init$) ou le paramètre $max{_}pop$.

        \subsubsection{Initialisation}
        
        Une multitudes d'initialisation sont possibles pour cet algorithme. On pourrait, bien que nous ne le faisons pas dans cet section, initialiser notre algorithme avec une approximation déjà faite. Le but étant que notre algorithme soit appliqué sur notre jeu de donnée sans étude préalable, on a opté pour une initialisation sans approximation préalable. Les graphes suivants montrent en particulier que l'initialisation aléatoire est la meilleure. ({\color{red} best b2 evaluation genetic random init})  ({\color{red} best b2 evaluation genetic ones init})  ({\color{red} best b2 evaluation genetic zeros init})
        ...
        On considère donc une initialisation aléatoire. Le graphe suivant nous permet de comprendre qu'à partir d'un certain palier, changer la valeur de $max{_}pop$ n'apporte rien à l'algorithme, tout en le ralentissant fortement. ({\color{red} best b2 evaluation diff genetic max{_}pop 15 30})

        En revanche, le graphe suivant nous montre qu'avec une population maximale trop faible, les barres d'erreurs deviennent relativement grandes. Cela signifie que l'algorithme est moins efficace en général. ({\color{red} best b2 evaluation genetic max{_}pop 5})

        Ainsi, une population maximale de 15 semble être la valeur optimale pour cet algorithme.


        De ce qui précède, peu importe les circonstances, les paramètres optimaux pour l'algorithme génétique sont $init = random$ et $max{_}pop = 15$.
        ...
        ({\color{red} best b2 evaluation diff genetic multiple})
